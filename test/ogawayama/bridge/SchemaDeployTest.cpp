...
protected:
    std::unique_ptr<database_mock> database_mock_{};  // NOLINT(cppcoreguidelines-non-private-member-variables-in-classes,misc-non-private-member-variables-in-classes)
};

TEST_F(SchemaDeployTest, basic_flow) {  // NOLINT(readability-function-cognitive-complexity,google-readability-avoid-underscore-in-googletest-name)
    ...
    EXPECT_EQ(dynamic_cast<takatori::value::int4 const&>(*(c1.default_value().element<yugawara::storage::column_value_kind::immediate>())).get(),
              123);
    // column 2
    auto& c2 = columns.at(2);
    EXPECT_EQ(c2.simple_name(), "c1");
    EXPECT_EQ(c2.type().kind(), takatori::type::type_kind::int8);
    EXPECT_EQ(dynamic_cast<takatori::value::int8 const&>(*(c2.default_value().element<yugawara::storage::column_value_kind::immediate>())).get(),
              123456789);
    ...
    EXPECT_EQ(dynamic_cast<takatori::value::float4 const&>(*(c3.default_value().element<yugawara::storage::column_value_kind::immediate>())).get(),
    ...
    EXPECT_EQ(dynamic_cast<takatori::value::float8 const&>(*(c4.default_value().element<yugawara::storage::column_value_kind::immediate>())).get(),
    ...
    EXPECT_EQ(dynamic_cast<takatori::value::character const&>(*(c5.default_value().element<yugawara::storage::column_value_kind::immediate>())).get(),
              "fixed length default");
    EXPECT_EQ(c5.criteria().nullity(), yugawara::variable::nullity(true));
    auto& c5c = dynamic_cast<takatori::type::character const&>(c5.type());
    ...
    EXPECT_EQ(dynamic_cast<takatori::value::character const&>(*(c6.default_value().element<yugawara::storage::column_value_kind::immediate>())).get(),
              "variable length default");
    EXPECT_EQ(c6.criteria().nullity(), yugawara::variable::nullity(true));
    auto& c6c = dynamic_cast<takatori::type::character const&>(c6.type());
    ...
    EXPECT_EQ(dynamic_cast<takatori::value::date const&>(*(c7.default_value().element<yugawara::storage::column_value_kind::immediate>())).get(),
    ...
    EXPECT_EQ(dynamic_cast<takatori::value::time_of_day const&>(*(c8.default_value().element<yugawara::storage::column_value_kind::immediate>())).get(),
    ...
    EXPECT_EQ(dynamic_cast<takatori::value::time_point const&>(*(c9.default_value().element<yugawara::storage::column_value_kind::immediate>())).get(),
