...
#define PAYMENT_1 statements[0].prepared
#define PAYMENT_2 statements[1].prepared
#define PAYMENT_3 statements[2].prepared
#define PAYMENT_4 statements[3].prepared
#define PAYMENT_5 statements[4].prepared
#define PAYMENT_6 statements[5].prepared
#define PAYMENT_7_GC statements[6].prepared
#define PAYMENT_7_BC statements[7].prepared
// #define PAYMENT_8 statements[8].prepared
#define PAYMENT_51 statements[8].prepared
#define PAYMENT_52 statements[9].prepared
...
int
transaction_payment(ConnectionPtr::element_type *connection, randomGeneratorClass *randomGenerator, std::uint16_t warehouse_low, std::uint16_t warehouse_high, [[maybe_unused]] tpcc_profiler *profiler)
{
	...
	plan_queries(statements, connection);
        ...
        if(transaction->execute_query(PAYMENT_1.get(), result_set) != ERROR_CODE::OK) {
        ...
        if(transaction->execute_statement(PAYMENT_2.get()) != ERROR_CODE::OK) {
            transaction->rollback();
            PG_RETURN_INT32(-1);
        }
        ...
        if(transaction->execute_query(PAYMENT_3.get(), result_set) != ERROR_CODE::OK) {
            elog(ERROR, "execute_query failed");
        }
        //                if(result_set->get_metadata(metadata) != ERROR_CODE::OK) {
        //                    elog(ERROR, "get_metadata failed");
        ...
        if(transaction->execute_statement(PAYMENT_4.get()) != ERROR_CODE::OK) {
            transaction->rollback();
            PG_RETURN_INT32(-1);
        }
        ...
        if (c_id == 0) {
            ...
            if(transaction->execute_query(PAYMENT_51.get(), result_set) != ERROR_CODE::OK) {
                elog(ERROR, "execute_query failed");
            }
            //                if(result_set->get_metadata(metadata) != ERROR_CODE::OK) {
            //                    elog(ERROR, "get_metadata failed");
            ...
            if(transaction->execute_query(PAYMENT_52.get(), result_set) != ERROR_CODE::OK) {
                elog(ERROR, "execute_query failed");
            }
            //                if(result_set->get_metadata(metadata) != ERROR_CODE::OK) {
            //                    elog(ERROR, "get_metadata failed");
        ...
        if(transaction->execute_query(PAYMENT_6.get(), result_set) != ERROR_CODE::OK) {
            elog(ERROR, "execute_query failed");
        }
        //                if(result_set->get_metadata(metadata) != ERROR_CODE::OK) {
        //                    elog(ERROR, "get_metadata failed");
	...
	if (c_credit[0] == 'G') {
            ...
            if(transaction->execute_statement(PAYMENT_7_GC.get()) != ERROR_CODE::OK) {
                transaction->rollback();
                PG_RETURN_INT32(-1);
            }
	} else {
		...
		sprintf(my_c_data, "%d %d %d %d %d %f %s ", my_c_id, c_d_id,
                ...
                if(transaction->execute_statement(PAYMENT_7_BC.get()) != ERROR_CODE::OK) {
                    transaction->rollback();
                    PG_RETURN_INT32(-1);
                }
	}
        ...
        if(transaction->execute_statement(PAYMENT_8.get()) != ERROR_CODE::OK) {
            transaction->rollback();
            PG_RETURN_INT32(-1);
        }
#endif
...
#if 0
int
transaction_payment(Database *db, randomGeneratorClass *randomGenerator, std::uint16_t warehouse_low, std::uint16_t warehouse_high, tpcc_profiler *profiler)
{
    paymentParams params = {};
    params.w_id = randomGenerator->uniformWithin(warehouse_low, warehouse_high);
    params.d_id = randomGenerator->uniformWithin(1, scale::districts);
    ...
    static auto prepared_update_warehouse = db->prepare(
            "UPDATE "
            "WAREHOUSE SET "
    ...
    static auto prepared_select_warehouse = db->prepare(
            "SELECT w_street_1, w_street_2, w_city, w_state, w_zip, w_name FROM WAREHOUSE "
            "WHERE "
            "w_id = :w_id"
    );

    static auto prepared_update_district = db->prepare(
            "UPDATE "
            "DISTRICT SET "
    ...
    static auto prepared_count_customer_secondary = db->prepare(
            "SELECT COUNT(c_id) FROM CUSTOMER_SECONDARY "
            "WHERE "
            "c_w_id = :c_w_id AND "
            "c_d_id = :c_d_id AND "
    ...
    static auto prepared_cid_from_customer_secondary = db->prepare(
            "SELECT c_id FROM CUSTOMER_SECONDARY "
            "WHERE "
            "c_w_id = :c_w_id AND "
            "c_d_id = :c_d_id AND "
    ...
    static auto prepared_select_customer = db->prepare(
    ...
    static auto prepared_cdata_from_customer = db->prepare(
    ...
    static auto prepared_update_customer_balance_and_data = db->prepare(
            "UPDATE "
            "CUSTOMER SET "
            "c_balance = :c_balance ,"
            "c_data = :c_data WHERE "
    ...
    static auto prepared_update_customer_balance = db->prepare(
            "UPDATE "
            "CUSTOMER SET "
    ...
    db->execute_transaction([&](Context *context) {
            auto finally = scope_guard([&profiler](){ profiler->mark_payment_end(); });
            ...
            try {
                ...
                ps->setDouble("h_amount", params.h_amount);
                ps->setInt("w_id", static_cast<std::int32_t>(params.w_id));
                context->execute_statement(prepared_update_warehouse.get(), std::move(ps));
                ...
                ps->setInt("w_id", static_cast<std::int32_t>(params.w_id));
                auto itrp1 = context->execute_query(prepared_select_warehouse.get(), std::move(ps));
                Closer result_closerp1{*itrp1};
                auto rowp1 = itrp1->next();
                if (rowp1 == nullptr) return Response::ERROR;
                VARCHAR10 w_name = ""; copyFromStringView<VARCHAR10>(static_cast<char *>(w_name), rowp1, 0);
                VARCHAR20 w_street_1 = ""; copyFromStringView<VARCHAR20>(static_cast<char *>(w_street_1), rowp1, 1);
                VARCHAR20 w_street_2 = ""; copyFromStringView<VARCHAR20>(static_cast<char *>(w_street_2), rowp1, 2);
                VARCHAR20 w_city = ""; copyFromStringView<VARCHAR20>(static_cast<char *>(w_city), rowp1, 3);
                CHAR2 w_state = ""; copyFromStringView<CHAR2>(static_cast<char *>(w_state), rowp1, 4);
                CHAR9 w_zip = ""; copyFromStringView<CHAR9>(static_cast<char *>(w_zip), rowp1, 5);
                if (itrp1->next() != nullptr) return Response::ERROR;
                itrp1->close();
                ...
                context->execute_statement(prepared_update_district.get(), std::move(ps));
                ...
                ps->setInt("d_w_id", static_cast<std::int32_t>(params.w_id));
                ps->setInt("d_id", static_cast<std::int32_t>(params.d_id));
                auto itrp2 = context->execute_query(prepared_select_district.get(), std::move(ps));

                Closer result_closerp2{*itrp2};
                auto rowp2 = itrp2->next();
                if (rowp2 == nullptr) return Response::ERROR;
                VARCHAR10 d_name = ""; copyFromStringView<VARCHAR10>(static_cast<char *>(d_name), rowp2, 0);
                VARCHAR20 d_street_1 = ""; copyFromStringView<VARCHAR20>(static_cast<char *>(d_street_1), rowp2, 1);
                VARCHAR20 d_street_2 = ""; copyFromStringView<VARCHAR20>(static_cast<char *>(d_street_2), rowp2, 2);
                VARCHAR20 d_city = ""; copyFromStringView<VARCHAR20>(static_cast<char *>(d_city), rowp2, 3);
                CHAR2 d_state = ""; copyFromStringView<CHAR2>(static_cast<char *>(d_state), rowp2, 4);
                CHAR9 d_zip = ""; copyFromStringView<CHAR9>(static_cast<char *>(d_zip), rowp2, 5);
                if (itrp2->next() != nullptr) return Response::ERROR;
                itrp2->close();
                ...
                if (params.byname) {
                    ...
                    //
                    ps = PreparedStatement::Parameters::get();
                    ps->setInt("c_w_id", static_cast<std::int32_t>(params.w_id));
                    ps->setInt("c_d_id", static_cast<std::int32_t>(params.d_id));
                    ps->setString("c_last", params.c_last);
                    auto itrp3 = context->execute_query(prepared_count_customer_secondary.get(), std::move(ps));
                    Closer result_closerp3{*itrp3};
                    auto rowp3 = itrp3->next();
                    if (rowp3 == nullptr) return Response::ERROR;
                    int namecnt = rowp3->get<int32_t>(0);
                    if (itrp3->next() != nullptr) return Response::ERROR;
                    itrp3->close();
                    if (namecnt == 0) return Response::COMMIT;  // this is a correct result
                    ...
                    ps->setInt("c_d_id", static_cast<std::int32_t>(params.d_id));
                    ps->setString("c_last", params.c_last);
                    auto itrp31 = context->execute_query(prepared_cid_from_customer_secondary.get(), std::move(ps));
                    Closer result_closerp31{*itrp31};
                    if (namecnt%2 > 0) { namecnt++; }
                    auto rowp31 = itrp31->next();
                    for (int n=1; n<namecnt/2; n++) {
                        rowp31 = itrp31->next();
                    }
                    c_id = rowp31->get<INTEGER>(0);
                    itrp31->close();
                } else {
                    c_id = params.c_id;
                }
                ...
                ps->setInt("c_w_id", static_cast<std::int32_t>(params.w_id));
                ps->setInt("c_d_id", static_cast<std::int32_t>(params.d_id));
                ps->setInt("c_id", static_cast<std::int32_t>(c_id));
                auto itrp4 = context->execute_query(prepared_select_customer.get(), std::move(ps));
                Closer result_closerp4{*itrp4};
                auto rowp4 = itrp4->next();
                if (rowp4 == nullptr) return Response::ERROR;
                VARCHAR16 c_first = ""; copyFromStringView<VARCHAR16>(static_cast<char *>(c_first), rowp4, 0);
                CHAR2 c_middle = ""; copyFromStringView<CHAR2>(static_cast<char *>(c_middle), rowp4, 1);
                VARCHAR16 c_last = ""; copyFromStringView<VARCHAR16>(static_cast<char *>(c_last), rowp4, 2);
                VARCHAR20 c_street_1 = ""; copyFromStringView<VARCHAR20>(static_cast<char *>(c_street_1), rowp4, 3);
                VARCHAR20 c_street_2 = ""; copyFromStringView<VARCHAR20>(static_cast<char *>(c_street_2), rowp4, 4);
                VARCHAR20 c_city = ""; copyFromStringView<VARCHAR20>(static_cast<char *>(c_city), rowp4, 5);
                CHAR2 c_state = ""; copyFromStringView<CHAR2>(static_cast<char *>(c_state), rowp4, 6);
                CHAR9 c_zip = ""; copyFromStringView<CHAR9>(static_cast<char *>(c_zip), rowp4, 7);
                //      CHAR16 c_phone = ""; copyFromStringView<VARCHAR20>(c_street_1, rowp4, 8);
                CHAR2 c_credit = ""; copyFromStringView<CHAR2>(static_cast<char *>(c_credit), rowp4, 9);
                //      DOUBLE c_credit_lim = rowp4->get<DOUBLE>(10);
                //      DOUBLE c_discount = rowp4->get<DOUBLE>(11);
                DOUBLE c_balance = rowp4->get<DOUBLE>(12);
                TIMESTAMP c_since = ""; copyFromStringView<TIMESTAMP>(static_cast<char *>(c_since), rowp4, 13);
                if (itrp4->next() != nullptr) return Response::ERROR;
                itrp4->close();
                ...
                c_balance += params.h_amount;
                if (strstr(static_cast<char *>(c_credit), "BC") ) {
                    ...
                    ps->setInt("c_d_id", static_cast<std::int32_t>(params.d_id));
                    ps->setInt("c_id", static_cast<std::int32_t>(c_id));
                    auto itrp5 = context->execute_query(prepared_cdata_from_customer.get(), std::move(ps));
                    Closer result_closerp5{*itrp5};
                    auto rowp5 = itrp5->next();
                    if (rowp5 == nullptr) return Response::ERROR;
                    VARCHAR500 c_data = ""; copyFromStringView<VARCHAR500>(static_cast<char *>(c_data), rowp5, 0);
                    if (itrp5->next() != nullptr) return Response::ERROR;
                    itrp5->close();
                    ...
                    //
                    VARCHAR500 c_new_data;
                    sprintf(static_cast<char *>(c_new_data),"| %4d %2d %4d %2d %4d $%7.2f %12s %24s",	// NOLINT
                            c_id,params.d_id,params.w_id,params.d_id,params.w_id,params.h_amount,		// NOLINT
                            static_cast<char *>(params.h_date), static_cast<char *>(params.h_data));	// NOLINT
                    strncat(static_cast<char *>(c_new_data),static_cast<char *>(c_data),500-strlen(static_cast<char *>(c_new_data)));
                    ...
                    //
                    ps = PreparedStatement::Parameters::get();
                    ps->setDouble("c_balance", c_balance);
                    ps->setString("c_data", c_new_data);
                    ps->setInt("c_w_id", static_cast<std::int32_t>(params.w_id));
                    ps->setInt("c_d_id", static_cast<std::int32_t>(params.d_id));
                    ps->setInt("c_id", static_cast<std::int32_t>(c_id));
                    context->execute_statement(prepared_update_customer_balance_and_data.get(), std::move(ps));
                    // }
                    // else
                    // {
                    //
                    ...
                    ps->setInt("c_d_id", static_cast<std::int32_t>(params.d_id));
                    ps->setInt("c_id", static_cast<std::int32_t>(c_id));
                    context->execute_statement(prepared_update_customer_balance.get(), std::move(ps));
                    // }
                ...
                //
                // We don't take care of the history table. 
